...
public class DiscreteSimulation {
    ...
    public int getWaitTimeOnFloor() {
        return waitTimeOnFloor;
    }
    ...
    public DiscreteSimulation(List<Elevator> elevatorList, int waitTimeOnFloor,
            int timeBetweenFloors, SchedulingAlgorithm scheduler, int maxFloor){
    ...
    public void runSimulation(final List<Passenger> passengerEntryList){
        ...
        while(true){
            if(passengerCounter >= passengerEntryList.size()){
                break; // TODO: Check logic. Last passenger might cause issue
            }
            while(passengerCounter < passengerEntryList.size() && currentTime == passengerEntryList.get(passengerCounter).getStartTime()){
                ...
                int liftNo = scheduler.assignLiftToPassenger(processingPassenger, elevatorList, this); // We call implementation
                passengerCounter++;
            }
        ...
        for(Elevator elevator : elevatorList){
            if(elevator.getWaitingFor() >= 0){
                ...
                if(elevator.getWaitingFor() == 0 ){
                    ...
                    passengerList = elevator.getFloorsGoToList().get(0).getIncomingPassengerList();
