...
api_url = st.text_input("Please share your Sheet **API** link here", value="https://api.steinhq.com/v1/storages/5f619d225d3cdc44fcd7d4b1")
sheet = st.text_input("Please input your sheet name here:", value="Queries150")
col_name = st.text_input("What is the name of the column which has user queries?", value="QueryText")
...
st.write("Use a larger number for quick analysis in the beginning and reduce when you are going for depth")
min_token_count = st.number_input("Analyze sentences which have atleast how many tokens?", value=2, min_value=2, max_value=10)
...
if st.checkbox("Explore Word Frequencies"):
    ...
    min_freq_count = st.number_input("What is the minimum frequency of words you want to see?", value=3, min_value=2, max_value=10)
...
if st.checkbox("Explore Word Sets"):
    ...
    def make_doc_term_matrix(corpus: textacy.Corpus, min_freq_count:int, ngrams:int = 1, entities:bool = True):
    ...
    vectorizer, doc_term_matrix = make_doc_term_matrix(corpus, min_freq_count=min_freq_count, ngrams=ngrams)
    repr(doc_term_matrix)


    n_topics = st.number_input("How many topics do you want to explore?", value =  3, min_value=2,max_value=10)
...
if st.checkbox("(Alpha) Words in Sentences Explorer"):
    "## Find Sentences with Specific Words"
    input_sentence = st.text_input("Enter the words (separated by space)",value="refund money", max_chars=300)
    ...
    similarity_cutoff = 0.00001
    import textacy.similarity
    from textacy.spacier.doc_extensions import get_preview

    for doc in corpus:
        score = textacy.similarity.jaccard(input_doc, doc, match_threshold=0.00000001)
...
if st.checkbox("Find Similar Sentences"):
    ...
    input_sentence = st.text_input("Enter your sentence to which you want to find similar examples",value="train ticket cancel", max_chars=300)
    input_doc = textacy.make_spacy_doc(input_sentence, lang="en")

    sent_count = st.slider('Maximum number of Similar sentences that you want', min_value=1, max_value=10, value=3)
    similarity_cutoff = st.slider('Similarity Cutoff', min_value=1, max_value=100, value=70) / 100.0
    import textacy.similarity
    from textacy.spacier.doc_extensions import get_preview
